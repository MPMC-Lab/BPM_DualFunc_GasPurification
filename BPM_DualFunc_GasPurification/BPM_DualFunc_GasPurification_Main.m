...
PriorOpts_Step1.Marginals(1).Parameters = [0 4];
...
PriorOpts_Step1.Marginals(3).Parameters = [-6 -4];
...
PriorOpts_Step1.Marginals(5).Parameters = [-5 -3];

PriorOpts_Step1.Marginals(6).Name = 'kd';
PriorOpts_Step1.Marginals(6).Type = 'Uniform';
PriorOpts_Step1.Marginals(6).Parameters = [-5 -3];
...
SigmaOpts_Step1.Marginals.Parameters = [0 (0.3)^2];
...
t_data = linspace(t_data_tmp(1),max(t_data_tmp),2*Ndata);% Time vector
...
PriorOpts_Step2.Marginals(5).Parameters = [-5 -3];

PriorOpts_Step2.Marginals(6).Name = 'kd';
PriorOpts_Step2.Marginals(6).Type = 'Uniform';
PriorOpts_Step2.Marginals(6).Parameters = [-5 -3];
% Creating the prior distribution as an input object
...
SigmaOpts_Step2.Marginals.Parameters = [0 (0.05)^2];
...
K_G = 1/inv_K;
Isotherm_param = [optpar_Step2(1),optpar_Step2(2)];
...
[t,c,~]= Model_DualFunc_GasPurification(dt,ngrid,dz,par,epsilon,cFeed(index),tf_plot,ModelNum,v,rho);
c_breakthrough=(c(ngrid,:)+c(ngrid+1,:))./2;
c_est = c_breakthrough/convert;
c_est = c_est/(1e-3 /34.08);
